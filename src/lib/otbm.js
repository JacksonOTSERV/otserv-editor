...
function parse(file) {
  ...
  pos++;
  const root = readNode(b, pos).node;
  // st.map: posKey -> nó do tile (SEM objeto leve duplicado — itens lidos sob demanda via itemsOf)
  const st = { header, root, map: new Map(), areaIndex: new Map(), zmin: 15, zmax: 0, towns: [], houses: 0, _dirty: false };
  ...
  const md = root.children.find((c) => c.type === OTBM_MAP_DATA);
  st.mapDataNode = md;
  if (md) {
    readMapAttrs(md.props, st);
    for (const ch of md.children) {
      if (ch.type === OTBM_TILE_AREA) {
        const bx = ch.props.readUInt16LE(0), by = ch.props.readUInt16LE(2), bz = ch.props[4];
        st.areaIndex.set(bx + ',' + by + ',' + bz, ch);
        for (const tn of ch.children) {
          ...
          tn._x = x; tn._y = y; tn._z = z; // posição no próprio nó (p/ render/edit)
          st.map.set(x + ',' + y + ',' + z, tn);
          if (z < st.zmin) st.zmin = z; if (z > st.zmax) st.zmax = z;
        }
      } else if (ch.type === OTBM_TOWNS) {
        for (const tn of ch.children) if (tn.type === OTBM_TOWN) { try { const id = tn.props.readUInt32LE(0); const r = readString(tn.props, 4); st.towns.push({ id, name: r.s, x: tn.props.readUInt16LE(r.o), y: tn.props.readUInt16LE(r.o + 2), z: tn.props[r.o + 4] }); } catch (e) {} }
      } else if (ch.type === OTBM_WAYPOINTS) {
        st.waypoints = st.waypoints || [];
        for (const wn of ch.children) { try { const r = readString(wn.props, 0); st.waypoints.push({ name: r.s, x: wn.props.readUInt16LE(r.o), y: wn.props.readUInt16LE(r.o + 2), z: wn.props[r.o + 4] }); } catch (e) {} }
      }
    }
  }
...
function writeNode(node, out) { out.push(NODE_START, node.type & 0xFF); escapeInto(node.props, out); for (const ch of node.children) writeNode(ch, out); out.push(NODE_END); }
...
function ensureTile(st, x, y, z) {
  ...
  tn = { type: OTBM_TILE, props: Buffer.from([x - bx, y - by]), children: [], _x: x, _y: y, _z: z };
  area.children.push(tn); st.map.set(key, tn);
  if (z < st.zmin) st.zmin = z; if (z > st.zmax) st.zmax = z;
  return tn;
}
...
function setTile(st, key, snap) {
  ...
  if (!snap) {
    if (cur) { const area = st.areaIndex.get((cur._x & 0xFF00) + ',' + (cur._y & 0xFF00) + ',' + cur._z); if (area) { const i = area.children.indexOf(cur); if (i >= 0) area.children.splice(i, 1); } st.map.delete(key); }
...
function removeEmptyTiles(st) {
  let rm = 0; for (const [k, n] of [...st.map]) { if (n.type === OTBM_HOUSETILE) continue; if (!getGround(n) && !n.children.some((c) => c.type === OTBM_ITEM)) { const area = st.areaIndex.get((n._x & 0xFF00) + ',' + (n._y & 0xFF00) + ',' + n._z); if (area) { const i = area.children.indexOf(n); if (i >= 0) area.children.splice(i, 1); } st.map.delete(k); rm++; } }
  if (rm) st._dirty = true; return rm;
}
// copia os tiles de outro mapa (other) p/ este, com deslocamento
function mergeRegion(st, other, offX, offY, offZ) {
  let n = 0;
